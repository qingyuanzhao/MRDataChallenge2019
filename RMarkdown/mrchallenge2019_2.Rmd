...
```{r}
sourceCpp("../MR-MCEM_submodule/EM.cpp")
...
# ```{r}
# pos_beta_ind <- which(beta_est > 0)
# pos_beta_ind
# beta_est[pos_beta_ind]
# ```
# 
# ```{r}
# pos_beta_SNPs <- data$SNP[pos_beta_ind]
# data.pos_beta <- combined.data %>% filter(SNP %in% pos_beta_SNPs)
# data.pos_beta$pval_m_ldl_c_kettunen
# ```
# * Two of the SNPs with positive $\hat{\beta}_i$ are highly associated with large LDL cholesterol.  
```

...
data <- as.data.frame(combined.data[, c("SNP", "pval_tg_teslovich",
                          "beta_tg_willer_mc", "se_tg_willer_mc",
                          "beta_cad", "se_cad")])
...
```{r}
# column names with tg in it 
colnames(pval.mtx2)[grep("tg", colnames(pval.mtx2))]
```
...
colnames(beta.mtx) <-
    sapply(colnames(beta.mtx), function(s) {a <- strsplit(s, "_")[[1]];
                                             paste(a[c(-1,-length(a))], collapse = "_")})
beta.mtx <- beta.mtx * sign(beta.mtx[,"idl_tg"])
